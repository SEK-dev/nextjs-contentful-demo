...
function JobsPageSideBarForm({
  ...
  const handleWorkFromChange = (e, option) => {
    console.log(e.target.checked, option)

    //TODO: send request and filter jobs
    if (e.target.checked) {
      setSideBarFormState(prevState => {
        const workFrom = [...prevState.workFrom]
        workFrom.push(option)
        return { ...prevState, workFrom }
      })
    } else {
      setSideBarFormState(prevState => {
        return {
          ...prevState,
          workFrom: prevState.workFrom.filter(wfrom => option != wfrom),
        }
      })
    }
  }
  ...
  const handleFeaturedJobsOnlyChange = checked => {
    ...
    //TODO: send request and filter jobs
    setSideBarFormState(prevState => {
      return { ...prevState, featuredJob: !prevState.featuredJob }
    })
  }
  ...
  const handleBaseSalaryRangesSelect = (e, option, bounds) => {
    ...
    if (e.target.checked) {
      setSideBarFormState(prevState => {
        ...
        const baseSalaryEuro = [...prevState.baseSalaryEuro]
        baseSalaryEuro.push(bounds.min)
        baseSalaryEuro.push(bounds.max)
      ...
      setSideBarFormState(prevState => {
        const newFormState = {
          ...
          baseSalaryEuro: prevState.baseSalaryEuro.filter(
      ...
      <div className='bg-white shadow-lg rounded-sm border border-slate-200 p-5'>
        <div className='grid md:grid-cols-2 xl:grid-cols-1 gap-6'>
          ...
          <Switch.Group as='div' className='flex items-center'>
            <Switch
              checked={sideBarFormState.featuredJob}
              onChange={handleFeaturedJobsOnlyChange}
              className={classNames(
                sideBarFormState.featuredJob ? 'bg-indigo-600' : 'bg-gray-200',
                  ...
                  <li key={option.value}>
                    <label className='flex items-center'>
                      <input
                        ...
                        onChange={e => handleWorkFromChange(e, option.value)}
                        checked={sideBarFormState.workFrom.includes(
                          option.value,
                        )}
                      />
